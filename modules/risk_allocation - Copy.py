...
def show_risk_allocation():
    ...
    if is_ai_enabled():
        ...
        prompt = """
        Scrape the latest articles from Yahoo Finance, CNBC, MarketWatch, Reuters, and Bloomberg. Focus on recent market performance (stocks, indices), economic indicators        (inflation, interest rates, GDP), volatility (VIX), and geopolitical events. 

        Analyze the tone (positive/negative/neutral) and summarize:
        1. Market sentiment (investor risk appetite)
        2. Key factors driving the sentiment (economic data, news, events)

        Provide a sentiment score (0-10) and a trend label (Bullish, Bearish, or Neutral). A score of 0 is extremely bearish, 10 is extremely bullish, and 5 is neutral. Trend           label should reflect overall market direction.
        """
