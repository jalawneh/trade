...
def show_profit_plan():
    ...
    top10 = st.session_state['top10'].head(15)
    target = 100
    budget = 3000
    ...
    # Try to use any remaining budget from any tier
    remaining_budget = budget - total_spent
    all_candidates = top10.sort_values('AI Recommendation (0–10)', ascending=False)

    for _, r in all_candidates.iterrows():
        price = r['Last Close ($)']
        move = max(3, r['Volatility (%)']) / 2
        max_shares = int(remaining_budget / price)
        if max_shares <= 0:
            continue
...
def show_gpt_summary():
    st.title("\U0001F916 GPT Market Summary")

    now = datetime.datetime.now().strftime('%B %d, %Y at %I:%M %p')
    today = datetime.datetime.now().strftime('%Y-%m-%d')

    st.info(f"🕒 Generating market summary for {today}... Please wait.")

    with st.spinner("Calling GPT with today's market prompt..."):
        prompt = (
            f"Generate a brief report by searching the web for major U.S. stock market news events occurring on {today}. "
            f"Include market-moving headlines, earnings reports, Fed announcements, geopolitical developments, economic data releases (e.g., jobs, CPI, GDP), and global events (e.g., China, oil, conflict).\n\n"
            f"Prioritize high-impact developments that may affect market sentiment, sector rotation, or volatility. "
            f"Summarize key takeaways in no more than four lines. Provide a very brief bullet summary. "
            f"Make sure to start the report with: 'Generated on {now}' followed by a colon."
        )
        result = call_openai_chat(prompt)

    st.markdown("### \U0001F9E0 GPT Output")
    st.success(result)
